...
async def _process_one(bot, call: dict):
    ...
    if provider_status in ("in-progress", "answered", "bridged"):
        new_cost = round((duration_seconds / 60) * config.CALL_RATE_PER_MIN, 4)
        delta = round(new_cost - float(call["cost"]), 4)
        ...
        if delta > 0 and float(user["balance"]) < delta:
            edesy.hangup_call(edesy_call_id)
            await _finalize(bot, call, duration_seconds, float(call["cost"]), status="completed",
    ...
    if provider_status in ("completed", "failed", "no-answer", "busy"):
        final_cost = round((duration_seconds / 60) * config.CALL_RATE_PER_MIN, 4)
        already_billed = float(call["cost"])
...
async def _finalize(bot, call: dict, duration_seconds: int, final_cost: float, status: str, reason: str = None):
    ...
    db.add_spent_and_minutes(telegram_id, final_cost, duration_min)

    ref_id = db.add_transaction(
        telegram_id, "call", final_cost, None, status,
        {"number": call["number"], "duration_min": duration_min, "via_api": bool(call.get("via_api"))},
    )
    ...
    text = texts.CALL_ENDED.format(
        duration=duration_min, amount=f"{final_cost:.2f}", currency=config.BOT_CURRENCY,
